...
/**
 * 【新增】直接修改用户余额
 * 允许用户直接设置余额为指定数值，而不是增量充值
 */
async function handleModifyBalance() {
  const currentBalance = state.globalSettings.userBalance || 0;
  
  const newBalanceStr = await showCustomPrompt(
    '修改余额', 
    `当前余额: ¥${currentBalance.toFixed(2)}<br>请输入新的余额数值:`, 
  ...
  if (isNaN(newBalance) || newBalance < 0) {
    await showCustomAlert('输入错误', '请输入有效的余额数值（不能为负数）！');
    return;
  }
  ...
  try {
    ...
    // 直接更新余额
    state.globalSettings.userBalance = newBalance;
    
    // 记录这次余额修改操作
    const transaction = {
      type: difference >= 0 ? 'income' : 'expense',
      amount: Math.abs(difference),
      description: `余额调整 (${currentBalance.toFixed(2)} → ${newBalance.toFixed(2)})`,
      timestamp: Date.now(),
    };
    ...
    await db.transaction('rw', db.globalSettings, db.userWalletTransactions, async () => {
      await db.globalSettings.put(state.globalSettings);
      await db.userWalletTransactions.add(transaction);
    });
    ...
    console.log(`用户余额已修改: ${currentBalance.toFixed(2)} → ${newBalance.toFixed(2)}`);
  } catch (error) {
    console.error('修改余额时出错:', error);
    await showCustomAlert('操作失败', `发生错误: ${error.message}`);
  }
}
...
function initTaobao() {
  ...
  document.getElementById('taobao-screen').addEventListener('click', async e => {
    ...
    // --- 我的页面 ---
      ...
      } else if (target.id === 'modify-balance-btn') {
